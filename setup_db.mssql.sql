...
-- Indexes used by the protected appointment dashboard and reminder queries.
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_Appointments_PreferredDate_Status' AND object_id = OBJECT_ID(N'dbo.Appointments'))
    CREATE INDEX IX_Appointments_PreferredDate_Status ON dbo.Appointments (PreferredDate, Status) INCLUDE (CreatedAt, PatientName, Phone, PreferredSlot);
GO
